...
class Optimization:
    ...
    def build_GaussianProcess_target_code(self):
        ...
        with open('evaluate_simulation.m','w') as fh:
            ...
            writeline('function overlap = evaluate_simulation(iteration, num_res)', fh)
            writeline('   ', fh)
            writeline('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%', fh)
            writeline('%%   ', fh)
            writeline('%%   ', fh)
            ...
            writeline("da = readdcd([num2str(iteration) '/optimization.dcd'],1:num_res);",fh)
            writeline('xyzmat=da(equil:end,:);', fh)
            

            writeline('   ', fh)
            writeline('% Number of frames', fh)
            writeline('nfr=size(xyzmat,1);', fh)


            writeline('for i=1:nfr', fh)
            writeline('  count = 0;', fh)

            writeline('   ', fh)
            writeline('  for j=1:nres', fh)
            writeline('    pos1(1:3,1) = xyzmat(i,1+3*(j-1):3+3*(j-1));', fh)

            writeline('   ', fh)
            writeline('    for k=1:nres', fh)
            writeline('      pos2(1:3,1) = xyzmat(i,1+3*(k-1):3+3*(k-1));', fh)
            writeline('      dx=(pos1(1,:)-pos2(1,:));', fh)

            writeline('   ', fh)
            writeline('      % periodicity in x dimension', fh)
            writeline('      if (abs(dx)>bl(1)*0.5)', fh)
            writeline('        dx=bl(1)-abs(dx);', fh)
            writeline('      end', fh)

            writeline('   ', fh)
            writeline('      % periodicity in y dimension', fh)
            writeline('      dy=(pos1(2,:)-pos2(2,:));', fh)
            writeline('      if (abs(dy)>bl(2)*0.5)', fh)
            writeline('        dy=bl(2)-abs(dy);', fh)
            writeline('      end', fh)

            writeline('   ', fh)
            writeline('      % periodicity in z dimension', fh)
            writeline('      dz=(pos1(3,:)-pos2(3,:));', fh)
            writeline('      if (abs(dz)>bl(3)*0.5)', fh)
            writeline('        dz=bl(3)-abs(dz);', fh)
            writeline('      end', fh)

            writeline('   ', writeline)
            writeline('      count = count+1;', fh)
            writeline('      distqf(i,count) = sqrt((dx).^2+(dy).^2+(dz).^2);', fh)
            writeline('      clear dx; clear dy; clear dz; clear pos2;', fh)
            writeline('    end', fh)
            writeline('    clear pos1;', fh)
            writeline('  end', fh)
            writeline('end', fh)
